...
async function main() {
  ...
  await page.reload({ waitUntil: 'networkidle' });
  assert((await page.textContent('#plJifen')) === '10', '新玩家有 10 游戏积分');
  assert((await page.evaluate(() => window.__pool.upgradeCue())) === true, '10 积分兑换树枝木棍');
  assert((await page.evaluate(() => window.__pool.state().level)) === 1, '球杆 Lv.1');
  assert((await page.evaluate(() => window.__pool.state().points)) === 0, '兑换花掉 10 积分');
  await page.evaluate(() => window.__pool.setPoints(200));
  assert((await page.evaluate(() => window.__pool.upgradeCue())) === true, '15 积分升到 Lv.2');
