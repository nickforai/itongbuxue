...
async function main() {
  ...
  await page.reload({ waitUntil: 'networkidle' });
  assert((await page.textContent('#plJifen')) === '1000', '新玩家有 1000 游戏积分');
  assert((await page.evaluate(() => window.__pool.buyCue('wood'))) === true, '1000 积分买木质球杆');
  assert((await page.evaluate(() => window.__pool.state().points)) === 0, '买球杆花掉 1000 积分');
  await page.evaluate(() => window.__pool.setPoints(15000));
  assert((await page.evaluate(() => window.__pool.buyCue('gold'))) === true, '10000 积分买金龙球杆');
  assert((await page.evaluate(() => window.__pool.state().points)) === 5000, '买金龙花掉 10000 积分');
